...
def y_to_x(y):
    j = 0
    x = []
    for i in range(I*N_t):
        x.append((Q_t_min[0] + Q_t_max[0])/2*y[j])
        j += 1
    for i in range(I*N_s):
        x.append((Q_s_min[0] + Q_s_max[0])/2*y[j])
        j += 1
    for i in range(I):
        x.append((E_g_min + E_g_max)/(a_ge*2.)*y[j])
        j += 1
    for i in range(I):
        x.append((S_b_min + S_b_max)/(a_b*2.)*y[j])
        j += 1
    return x
