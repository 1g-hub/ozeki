...
def f_sj(j,i):
    return x_s*y_s / (-a_s[j] * (x_s*y_s) ** 2 + b_s[j] * x_s*y_s + c_s[j])
...
for i in range(I):
    ...
    for j in range(N_s):
        f_sj_sum += f_sj(j, i)
    expr = a_gs * x_g*y_g + a_b * x_b*y_b - f_sj_sum - S_L[i] == S_rm[i]
    model.Eq_c.add(expr)
...
def ObjRule(model):
    f = 0.0
    for i in range(I):
        f += C_Er[i] * E_r(i) + C_Fr[i] * (x_g*y_g + x_b*y_b)
    return f
...
def E_r(i):
    ...
    return f_t + E_L[i] - a_ge * x_g*y_g + E_rm[i]
